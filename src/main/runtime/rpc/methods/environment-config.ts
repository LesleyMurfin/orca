...
const portableSettingsShape = {
  ...
  terminalScrollbackRows: z.number().finite().optional(),
  ...
  // Why: canonical key the renderer reads. The legacy `experimentalCompactWorktreeCards`
  // is deprecated on current main — Store.load() migrates it to `compactWorktreeCards`
  // and resets the legacy key to undefined each load, so allowlisting the legacy name
  // would be a silent no-op discarded on restart.
