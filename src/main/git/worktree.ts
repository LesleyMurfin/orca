...
// poller hot-loops failed spawns indefinitely (PRB-0003). Once a path is
// *confirmed* missing (stat -> ENOENT), back off and probe it at most once per
// window instead of on every poll. Any successful scan — or any local worktree
...
// Why: the `\0` separator must stay in lockstep with the prefix scan in
// clearRepoPathMissingForRepo, which retires every distro variant of a path.
 ...
 *  module-global map can't leak state across tests. */
...
export async function listWorktreeGraph(
  ...
  try {
    ...
    missingRepoPathBackoffUntil.delete(backoffKey)
    return graph
  } catch (err) {
    if (getErrorCode(err) === 'ENOENT') {
      try {
        ...
        if (getErrorCode(statErr) === 'ENOENT') {
          missingRepoPathBackoffUntil.set(backoffKey, Date.now() + MISSING_REPO_PATH_BACKOFF_MS)
