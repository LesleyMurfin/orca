...
# Cleaning up many settled workers at once (end-of-session sweep)? Release every
# currently reclaimable Dispatch in one call instead of looping --dispatch; --run
# scopes it like worker-list, and it's mutually exclusive with --dispatch:
orca orchestration worker-release --terminal-state reclaimable --run <runId> --json
...
Bulk release applies the same per-dispatch logic to every reclaimable Dispatch in scope — it never touches active, retained, release_pending, or already-released terminals, and one Dispatch erroring never aborts the rest. The JSON receipt totals `released`, `alreadyReleased`, `retained`, `failed`, and `releasePending` against `requested`, reports which Run scope resolved (`scope`: explicit `--run`, terminal-bound Run, or all Runs), and lists each outcome. Exit code is 1 only when `failed` is nonzero.
