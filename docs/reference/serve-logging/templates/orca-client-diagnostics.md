...
1. **Reach** the transport endpoint (`ws://<pairing-address>:<port>`).
...
```bash
# TCP reachability to the advertised pairing address (overlay IP, not always localhost)
nc -zv <pairing-address> <port>
...
The CLI dispatches to a **local unix socket** (`runtime.sock`, same host) or a **remote
...
# On the serve host: is that runtime id actually recorded in the runtime metadata?
...
orca --environment <id> status --json | jq -r .runtime.runtimeId        # client view
cat "${ORCA_USER_DATA_PATH:-$HOME/.config/orca}/orca-runtime.json" | jq -r .runtimeId  # serve truth
```
...
```bash
orca --version                                                           # client
grep '^ORCA_VERSION=' "$PREFIX/etc/instances/<instance>.env"             # serve (deployed version)
```
...
rejection in the serve journal rather than a clean error. Align versions, or record the
build you deployed in the instance's `ORCA_VERSION`.
...
set -eux
nc -zv <pairing-address> <port>
orca --environment <id> status --json | jq -e '.runtime.reachable == true'
test "$(orca --environment <id> status --json | jq -r .runtime.runtimeId)" = \
     "$(cat "${ORCA_USER_DATA_PATH:-$HOME/.config/orca}/orca-runtime.json" | jq -r .runtimeId)"
